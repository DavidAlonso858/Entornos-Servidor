...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // O mediante toSet
        Set<String> setCity = transactions.stream()
                .map(transaction -> transaction.getTrader().getCiudad())
                .collect(toSet());
        ...
        List<Trader> traderCambridge = transactions.stream()
                ...
                .filter(transaction -> "Cambridge".equals(transaction.getCiudad()))
        ...
        boolean milanTrader = transactions.stream()
                .map(Transaction::getTrader)
                .anyMatch(trader -> "Milan".equals(trader.getCiudad()));

        // 6 Imprime los valores de todas las transacciones de los traders que viven en Cambridge.
        transactions.stream()
                .filter(transaction -> "Cambridge".equals(transaction.getTrader().getCiudad()))
